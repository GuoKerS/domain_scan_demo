...
# external modules
from lib.Sublist3r.subbrute import subbrute
import dns.resolver
import requests
    ...
    def extract_domains(self, resp):
        ...
        try:
            for i in resp['data']:
                if i['type'] == 'domain':
                    subdomain = i['id']
                    # print('V: %s' % subdomain) # 每次获取的数据在这里，可以考虑传进队列，实时获取新提取到的域名然后工作
                    if not subdomain.endswith(self.domain):
                        continue
                    if subdomain not in self.subdomains and subdomain != self.domain:
                        if self.verbose:
    ...
    def extract_domains(self, resp):
        ...
        try:
            ...
            for link in links:
                ...
                if '<BR>' in subdomain:
                    sub = subdomain.splist('<BR>')
                    for i in sub:
                        if i not in self.subdomains and i != self.domain:
